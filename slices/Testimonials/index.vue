...
const testimonials = computed(() => {
	return props.slice.items
		...
		.filter(
			testimonial => isFilled.contentRelationship(testimonial)
...
<template>
	<Bounded :data-slice-type="slice.slice_type" :data-slice-variation="slice.variation">
		<PrismicText :field="slice.primary.heading" wrapper="h2"
		...
		<div class="grid lg:grid-cols-3 grid-cols-1 gap-8">
			<div v-for="testimonial of testimonials" :key="testimonial.data.name ?? ''"
				...
				<div class="flex items-center">
					<PrismicImage width="56" height="56" :field="testimonial.data.avatar"
						class="rounded-full mr-4 aspect-square" :imgix-params="{ ar: '1:1', fit: 'crop' }" />
					<div>
